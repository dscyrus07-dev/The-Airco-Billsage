...
Pydantic models for purchase invoice/voucher data validation and serialization.
Maps to vouchers table with voucher_type='purchase'.
"""
...
class PurchaseItemResponse(PurchaseItemCreate):
    """Schema for purchase line item response"""
    id: str
    voucher_id: str
    created_at: datetime
...
class PurchaseCreate(BaseModel):
    ...
    party_id: str = Field(..., description="Supplier/vendor ID")
    voucher_number: str = Field(..., max_length=50)
    voucher_date: date
    ref_number: Optional[str] = Field(None, max_length=100, description="Supplier invoice number")
    ...
    # GST details
    supply_type: Optional[str] = Field(None, description="B2B, B2C, etc.")
...
class PurchaseUpdate(BaseModel):
    """Schema for updating a purchase invoice"""
