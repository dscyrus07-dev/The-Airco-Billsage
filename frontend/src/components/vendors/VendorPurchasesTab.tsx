...
interface VendorPurchasesTabProps {
  vendorId: string;
  invoices: VendorInvoice[];
  isLoading?: boolean;
}

export default function VendorPurchasesTab({
  vendorId,
  invoices,
  isLoading = false,
}: VendorPurchasesTabProps) {
  ...
  const getAgingBadge = (aging: string) => {
    ...
    return (
      <Badge className={colors[aging as keyof typeof colors] || 'bg-gray-100 text-gray-800'}>
        {aging} days
      </Badge>
    );
  };
        ...
        {filteredInvoices.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">
              {invoices.length === 0 
                ? "This vendor doesn't have any invoices yet."
